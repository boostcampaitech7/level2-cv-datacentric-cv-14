...
from synth_utils import draw_bbox, make_document, Document
import os
import time
import json
...
# 디렉토리 생성
os.makedirs("./synth_data/img/train", exist_ok=True)
os.makedirs("./synth_data/train/ufo", exist_ok=True)

# UFO format json 데이터 초기화
ufo_data = {"images": {}}

# 이미지 개수가 100개가 될 때까지 생성
while len(os.listdir("./synth_data/img/train")) < 100:
    # 문서 생성 및 변형
    doc = make_document(get_words(30))
    perturb_document_inplace(doc)
    
    # 이미지 파일명 생성
    img_filename = f"synthetic_image_{time.strftime('%Y%m%d_%H%M%S')}_{len(os.listdir('./synth_data/img/train')):03d}.jpg"
    img_path = f"./synth_data/img/train/{img_filename}"
...
json_path = "./synth_data/train/ufo/train.json"
